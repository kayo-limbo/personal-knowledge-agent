  ...
  return (
    <main className="space-y-6">
      ...
      <form className="flex gap-2 rounded-xl border bg-card p-4 shadow-sm" method="get">
        <input className="min-w-0 flex-1 rounded-lg border border-border px-3 py-2 text-sm outline-none focus:border-gray-400" name="query" defaultValue={params.query} placeholder="按会话标题搜索" />
      ...
      <section className="space-y-3">
        ...
        {conversations.length === 0 ? (
          <div className="rounded-xl border border-dashed bg-card p-10 text-center text-sm text-muted-foreground">没有匹配的会话记录。</div>
        ) : conversations.map((conversation) => {
          const preview = conversation.messages[0]?.content.replace(/\s+/g, " ").trim();
          return (
            <article key={conversation.id} className="rounded-xl border bg-card p-5 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <Link className="shrink-0 rounded-lg bg-primary px-4 py-2 text-center text-sm font-medium text-primary-foreground hover:bg-primary/90" href={`/dashboard/chat?conversation=${encodeURIComponent(conversation.id)}`}>
                  继续对话
                </Link>
              </div>
              <div className="mt-4 grid gap-3 border-t pt-4 lg:grid-cols-[1fr_auto]">
                <form action={renameConversationAction} className="flex gap-2">
                  ...
                  <input className="min-w-0 flex-1 rounded-lg border border-border px-3 py-2 text-sm" name="title" defaultValue={conversation.title} maxLength={80} required aria-label="新会话标题" />
                  <button className="rounded-lg border px-3 py-2 text-sm font-medium hover:bg-muted/40" type="submit">重命名</button>
                </form>
                <details className="text-sm">
                  ...
                  <form action={deleteConversationAction} className="mt-2 flex items-center gap-2">
                    ...
                    <button className="rounded-lg bg-red-600 px-3 py-2 text-xs font-medium text-primary-foreground" type="submit">确认</button>
                  </form>
                </details>
              </div>
            </article>
