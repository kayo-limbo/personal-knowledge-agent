...
最新范围更新（2026-09-15）：用户明确要求 Embedding 升级并已提交功能（`aa0ddf1`）。已实现 pgvector 索引、响应后更新、混合检索、补建入口与评测脚本；独立本机 pgvector 数据库的三条 migration 与生命周期集成测试通过。尚无供应商 Key，中文效果、浏览器和公网验证待完成，未 push/部署。用户已取消自动生成代码解释文档，后续仅维护必要配置和验证记录。
