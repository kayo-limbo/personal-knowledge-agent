...
最新范围更新（2026-09-15）：用户明确要求开始 Embedding 升级，覆盖下文此前的延期决定。已实现 pgvector 索引、响应后更新、混合检索、补建入口与评测脚本；44 项测试、Lint、类型及临时 PostgreSQL 配置下生产构建通过。尚无供应商 Key，本机 Docker 引擎不可用；真实数据库集成、中文效果、浏览器和公网验证待完成，尚未提交发布。详见 `../features/embedding-hybrid-retrieval.md`。

最后更新：2026-09-09
用户最新决定：不制作 PPT、不录视频，改为准备固定数据并人工展示。下方原排期中有关 PPT/录像的条目仅为历史计划，以本条和 next-session-handoff.md 为准。
本轮本地收尾已完成：旧会话角色同步、中文输入保护、SSE 结束确认与离页取消、顶部搜索、聊天选用 Prompt、独立演示账号与知识。用户随后追加的 PDF/TXT/MD 文件导入与混合文本检索也已完成；39 项测试、Docker 生产构建及本地真实 Chrome 文件问答闭环通过，代码尚未提交发布。下一步是提交后由用户 push、检查 CI、部署并公网回归。
状态补充（2026-09-09）：`bcbf877` 已通过 GitHub Actions 并在 Render live。本地 Compose 旧镜像导致侧栏修复未生效，已重建应用；真实 Chrome ADMIN 六个导航页的长内容滚动验证通过。公网登录后的视觉与功能回归、固定演示材料仍待完成，详见 `next-session-handoff.md` 顶部更新。
验收日期：2026-09-13
规划周期：约 34 天
...
1. 文件上传和解析。（2026-09-09 已完成 PDF/TXT/MD、分块、混合文本检索和文件引用）
2. Embedding、向量数据库和 rerank。（没有 Embedding Key，继续延期；当前功能不能称为向量语义检索）
...
优先砍掉向量数据库、多环境部署、MCP 和 Multi-Agent，不能砍掉已完成的文件导入、知识检索、权限、稳定演示和文档。
...
默认推荐的下一个任务是：**完成本轮修复的提交与公网发布，并按 docs/acceptance/manual-demo.md 人工演练**。固定本地数据和操作步骤已准备，用户不需要 PPT 或录像。公网新版本与现场同类网络仍待检查；不自动 seed，不扩展新技术范围。
