...
最新范围更新（2026-09-15）：Embedding 原功能已在 Git 中；用户新增要求处理无答案误召回并更新 Flash。已加入有时限的证据核验、原文引文校验、取消传递与模型名兼容；40 道公开合成问题和本机真实普通/思考/SSE/持久化回归通过。新代码已按用户要求纳入本轮提交，待 push 与发布，无新增 migration。必要验证与发布记录见 next-session-handoff.md 顶部，不新增代码教学文档。
