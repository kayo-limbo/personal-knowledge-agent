...
当前做了消息条数、字符数、单条输入长度和最大输出限制，并默认选择 Flash 普通模式；另外使用 PostgreSQL 持久化每账号/全站每日请求配额，在调用模型前原子计数。字符数和请求数都不是准确 token，后续仍要使用供应商 usage 做成本统计。
...
18. **请求配额不等于 Token 预算**：当前用户/全站每日请求门禁能限制入口，但不同模型、思考模式、Agent 轮数和联网搜索成本不同，仍需 usage 统计、告警和预算熔断。
...
2. 在已有 PostgreSQL 每日请求配额上增加幂等键、usage 统计、告警和预算熔断。
...
1. **聊天基础加固**：已有用户/全站每日请求配额；继续记录模型、思考模式和 token usage，增加幂等键与消息状态。
