...
登录的 ADMIN 和 USER 可以创建、查看、编辑、收藏、标记公开意向及删除自己的 Prompt；GUEST 无权进入。聊天新会话现在可以选用个人 Prompt，详见 [聊天选用说明](chat-prompt-selection.md)。公开意向暂不产生对外分享权限。
...
聊天选择器、Conversation 关联和服务端模板注入已接通；模板变量和版本历史延期，验收前不扩展 Prompt 市场。
