...
- 搜索会产生额外模型 Token 成本。现有用户/全站每日聊天请求配额限制入口，但不等于搜索次数或 Token 预算；演示时仍要观察余额。
...
答：Agent 每轮都是新的上游 API 请求，而且 DeepSeek 兼容接口实测没有严格遵守一次内部搜索。应用要跨轮记录并移除工具；已有每日请求配额负责入口保护，搜索专用统计和真实 usage 仍待增加。
...
验证结果：21 个单元测试与本地真实 Agent/HTTP 调用先通过。随后 `11e29e5` 于 2026-09-08 在 Render 公网上线；强制联网样本产生 335 个 delta，首字约 2.8 秒，包含网页链接、SSE `done`，最终拼接文本与 Neon Message 一致。重新部署同一 SHA 后该会话仍可读取，随机测试账号及关联记录已精确清理。单次时延不是性能保证。
