...
(async () => {
  ...
  try {
    ...
    await fill('textarea', '请用 Markdown 列表写两条学习建议，并给出一行 JavaScript 代码。');
    ...
    assert(await evaluate('!!document.querySelector("article ul") && !!document.querySelector("article code")'));
    ...
    console.log('PASS: IME Enter does not send; real DeepSeek Prompt answer, Markdown and persistence.');
    ...
    await fill('select[aria-label="搜索范围"]', '/dashboard/history'); await fill('input[aria-label="搜索关键词"]', 'Markdown');
    await evaluate('document.querySelector("form[role=search] button").click()');
    await waitFor('location.search.includes("query=Markdown")');
    assert(await evaluate('document.body.innerText.includes("请用 Markdown")'));
